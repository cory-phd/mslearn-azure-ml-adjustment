...
az ml compute create --name ${COMPUTE_INSTANCE} --size STANDARD_D2DS_V4 --type ComputeInstance 

# Create compute cluster
echo "Creating a compute cluster with name: " $COMPUTE_CLUSTER
az ml compute create --name ${COMPUTE_CLUSTER} --size STANDARD_D2DS_V4 --max-instances 2 --type AmlCompute 
